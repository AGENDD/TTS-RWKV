...
__device__ inline float to_float(const fp & u) { return u; } // 修改：将 __bfloat162float 替换为直接返回 u
__device__ inline fp to_fp(const float & u) { return u; } // 修改：将 __float2bfloat16_rn 替换为直接返回 u

typedef fp * __restrict__ F_; // 修改：将 bf 替换为 fp

__global__ void forward_kernel(int T, int H, F_ w_, F_ q_, F_ k_, F_ v_, F_ a_, F_ b_, fp* y_, float* s_, float* sa_) { // 修改：将 bf* 替换为 fp*
    constexpr int C = _C_;
...
#pragma unroll
        ...
        y_[ind] = to_fp(y); // 修改：将 to_bf 替换为 to_fp
...
__global__ void backward_kernel(int T, int H, F_ w_, F_ q_, F_ k_, F_ v_, F_ a_, F_ b_, F_ dy_, float * __restrict__ s_, float * __restrict__ sa_, fp* dw_, fp* dq_, fp* dk_, fp* dv_, fp* da_, fp* db_) { // 修改：将 bf* 替换为 fp*
    constexpr int C = _C_;
    int bb = blockIdx.y, hh = blockIdx.x, i = threadIdx.x;
...
#pragma unroll
        ...
        dq_[ind] = to_fp(dq); // 修改：将 to_bf 替换为 to_fp
...
#pragma unroll
        ...
        dw_[ind] = to_fp(dw * wi * wi_fac); // 修改：将 to_bf 替换为 to_fp
        dk_[ind] = to_fp(dk); // 修改：将 to_bf 替换为 to_fp
        dv_[ind] = to_fp(dv); // 修改：将 to_bf 替换为 to_fp
        db_[ind] = to_fp(db); // 修改：将 to_bf 替换为 to_fp
...
#pragma unroll
        ...
        da_[ind] = to_fp(da); // 修改：将 to_bf 替换为 to_fp
